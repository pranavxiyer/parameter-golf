...
class Hyperparameters:
    ...
    mlp_rank: int = int(os.environ.get("MLP_RANK", 32))
...
# tuple of strings for parameters to update via adam
...
# tuple of strings for parameters we do not quantize (keep float32)
...
class AdapterMLP(nn.Module):
    # Random learned adapter MLP 
    def __init__(self, dim: int, mlp_mult: int, rank: int, seed: int):
        super().__init__()
        self.dim = dim
        self.hidden = dim * mlp_mult
        self.seed = seed
        self.A_fc   = nn.Linear(rank, self.hidden, bias=False).weight.astype(mx.float32) * 0.01 # (hidden, rank)
        self.B_fc   = mx.zeros((rank, dim), dtype=mx.float32) # (rank, dim)
        self.A_proj = nn.Linear(rank, dim, bias=False).weight.astype(mx.float32) * 0.01 # (dim, rank)
        self.B_proj = mx.zeros((rank, self.hidden), dtype=mx.float32) # (rank, hidden)
    
    def __call__(self, x: mx.array) -> mx.array:
        W_fc   = mx.random.normal((self.hidden, self.dim), key=mx.random.key(self.seed)) / (self.dim ** 0.5) # (hidden, dim)
        W_proj = mx.random.normal((self.dim, self.hidden), key=mx.random.key(self.seed + 1)) / (self.hidden ** 0.5) # (dim, hidden)
        fc   = (W_fc   + self.A_fc   @ self.B_fc).astype(x.dtype) # (hidden, dim)
        proj = (W_proj + self.A_proj @ self.B_proj).astype(x.dtype) # (dim, hidden)
        x = nn.relu(x @ fc.T) # (B, T, hidden)
        return (x * x) @ proj.T # (B, T, dim)
...
class Block(nn.Module):
    def __init__(
        ...
        self.mlp = AdapterMLP(dim, mlp_mult, rank=mlp_rank, seed=block_idx*2)
...
class GPT(nn.Module):
    ...
    def __init__(self, vocab_size: int, num_layers: int, dim: int, num_heads: int, num_kv_heads: int, mlp_mult: int, 
                 mlp_rank: int, logit_chunk_tokens: int, logit_softcap: float, rope_base: float, 
        ...
        self.blocks = [
            Block(dim, num_heads, num_kv_heads, mlp_mult, mlp_rank, rope_base, qk_gain_init, block_idx=i)
...
def main() -> None:
    ...
    while True:
        ...
        run_val = (args.val_loss_every > 0 and step % args.val_loss_every == 0) or (last_step and not args.skip_final_val)
        if run_val:
